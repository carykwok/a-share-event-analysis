...
- **11 大类模板库**：10 大普通事件 × 20 子类，外加 11A/11B/11C 人物讲话类（权威口径、产业领袖、市场观点）。
...
**`SKILL.md` 为自包含单文件**（含完整工作流 + 五条合规红线 + 20 个普通事件模板 + 3 个人物讲话类写作框架），任何平台直接加载本文件即可：
...
| 11 | 人物讲话类 | 权威人物口径变化 | 产业领袖/公司信号 | 市场观点信号 |

## 合规红线（对客端强制）

本 skill 已内嵌对客合规红线，所有模板均已按下列原则改造：

**五条绝对禁止**：
1. 不写具体股票名称+代码（如"中国卫星(600118)"这类组合不出现）
2. 不做行情绝对性走势判断（禁用"必然/一定/见底/反转/突破"等词）
3. 不给投资操作建议（禁用"买入/建仓/超配/低配/规避/建议配置"等词）
4. 不做板块估值区间判断（禁用"板块 PE 高位/估值偏高/估值泡沫"等断言）
5. 人物讲话类不编造引文：加引号内容必须逐字可溯源，跨语言讲话注明中文来源
...
HTML 页面的 UI 规范参考 Anthropic [frontend-design](https://github.com/anthropics/skills/tree/main/skills/frontend-design) skill，**强制 7 楼层结构 + 20 个普通事件子类及 3 个人物讲话类色彩主题 + 避免 AI 套路化美学**（详见 SKILL.md Step 6）。
...
**脚本能力**：解析 YAML frontmatter → 按 `category` 自动匹配 20 个普通事件子类及 3 个人物讲话类色彩主题 → 按 Hero + N 楼层 + Footer 结构渲染 HTML → 自动编号存入 `export/YYYY-MM-DD/NN-<slug>.html`。输出含 gradient mesh、噪点纹理、非对称卡片、staggered 入场动画、自定义 Fraunces/JetBrains Mono 字体，严格规避 Inter/Roboto 与白底紫渐变等 AI 套路。
...
- [07 吴清在证监会系统工作会议讲话](./examples/11A-wuqing-csrc-2026.md) — 11A 权威人物口径变化
- [08 曾毓群谈固态电池与反内卷](./examples/11B-zengyuqun-solid-state-battery.md) — 11B 产业领袖/公司信号
- [09 黄仁勋 GTC 2026 主题演讲](./examples/11B-huangrenxun-gtc2026.md) — 11B 产业领袖/公司信号
- [10 王兴兴谈人形机器人量产](./examples/11B-wangxingxing-humanoid-robot.md) — 11B 产业领袖/公司信号
- [11 陈果团队 2026 中期策略](./examples/11C-chenguo-strategy-outlook.md) — 11C 市场观点信号
